...
main() {
  ...
  test('Should edit a post', () async {
    ...
    when(repository.editPost(post)).thenAnswer((_) async => Right(post));
    expect(usecase(post), completes);
  });

  test('Should return a EditPostError', () async {
    when(repository.editPost(any))
        .thenAnswer((_) async => Left(EditPostError()));
    final result = await usecase(Post());
    expect(result.leftMap((l) => l is EditPostError), Left(true));
  });

  test('Should return a EditPostError when post content is invalid', () async {
    final userLogged =
        User(email: "eronildoj@gmail.com", name: "Eronildo", photo: "photo");
  ...
  test('Should return a EditPostError when is not post owner', () async {
